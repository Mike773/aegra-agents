...
Звёзды. Узел с непустым ``star_received`` — это ЗВЕЗДА: именной показатель
(«Звезда качества»), у которого числа нет вовсе (ни fact, ни plan) — только
результат «получена / не получена» на последнюю дату. Звёзд у сотрудника может
быть несколько, каждая независима. Её ``child_metrics`` — влияющие показатели:
обычные числовые метрики с ``is_star_metric=true`` (у прод-конвертера все даты
ребёнка лежат сиблингами под одним узлом звезды). ``is_star_metric`` на самой
звезде ничего не значит. Оба поля опциональны: их отсутствие (None) — не то же
самое, что False, см. ``_normalize_flag``.
...
ROW_FIELDS: tuple[str, ...] = (
    ...
    "influent_percent",
    "element",
    # Узел — звезда: результат «получена / не получена» вместо fact (числа у
    # звезды нет). NULL = поля не было (обычная числовая метрика).
    "star_received",
    # Метрика влияет на получение РОДИТЕЛЬСКОЙ звезды (через выполнение плана).
    # На самой звезде флаг ничего не значит.
    "is_star_metric",
)
...
def _has_binary_result(node: dict[str, Any]) -> bool:
    """Звезда приходит БЕЗ числового факта: её результат лежит в star_received.
    Такой узел нельзя выкидывать по правилу «нет факта — нет строки», иначе
    звезда молча исчезнет из датасета.

    Спасаем строку строго по star_received, а НЕ по is_star_metric: последний
    стоит на обычных числовых метриках, а числовой лист с пустым фактом — это
    ровно тот случай, ради которого правило отбрасывания и написано."""
    return _normalize_flag(node.get("star_received")) is not None


def has_star_fields(row: dict[str, Any]) -> bool:
    """Есть ли в РАЗОБРАННОЙ строке звёздные поля: сама звезда (star_received)
    либо показатель с пометкой «влияет на звезду». Единый предикат для всех
    гейтов (промпт-блоки v5 и v4): на обычном датасете — False."""
    if row.get("star_received") is not None:
        return True
    return bool(row.get("is_star_metric"))
...
def _walk(
    ...
    for node in metrics:
        ...
        # Исключение — звезда: у неё факта нет по определению.
        if (
            not children
