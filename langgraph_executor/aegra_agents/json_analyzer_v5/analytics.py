...
def _level_order(agg_rows: list[dict], ref_level: str | None = None) -> list[str]:
    ...
    # Позиция уровня в справочнике конфига; вне справочника — после всех
    # настроенных, там порядок определяет прежняя эвристика.
    configured = {code.strip().casefold(): i for i, code in enumerate(level_order())}
    order = sorted(
        info,
        key=lambda lv: (
            configured.get(str(lv).strip().casefold(), len(configured)),
...
def _node_header(row: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "measure_type": row.get("measure_type"),
        "fact": row.get("fact"),
        # ex — % выполнения плана, rr — RunRate; приходят не всегда (None = нет).
        "ex": row.get("ex"),
        "rr": row.get("rr"),
        "plan_status": row.get("plan_status"),
        "plan_dev_pct": row.get("plan_dev_pct"),
