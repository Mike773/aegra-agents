...
def build_tools(
    ...
    def get_metric(
        ...
        trend_status / pop_status ('улучшение'/'ухудшение'/'стабильно') и
        peer_status ('лучше_коллег'/'хуже_коллег'). Сырые trend ('рост'/'падение'),
        pop_change_pct и знак zscore — это направление ЗНАЧЕНИЯ, а не «хорошо/плохо»:
        для 'обратной' метрики рост значения = ухудшение.
        person — ФИО (или часть) либо табельный номер; date — период (YYYY-MM-DD).
    ...
    def compare(
        ...
        """Динамика метрики по периодам (поля pop_change_pct и trend, плюс
        вердикты с учётом направления pop_status и trend_status —
        'улучшение'/'ухудшение'/'стабильно') для одного человека. Оценивай по
        *_status, а не по знаку pop_change: для 'обратной' метрики рост значения =
    ...
    def metric_tree(
        ...
        pop_change_pct, pop_status, influent_percent). Динамику оценивай по
        trend_status/pop_status ('улучшение'/'ухудшение'), а не по сырому trend.
