...
# Каждый цикл react ≈ 2 суперстепа (модель + tools), плюс финал. При лимите 50
# реально достижимо ~24 вызова — меньше нового бюджета (32). Поднимаем, чтобы
# управляющим потолком был бюджет вызовов, а не рекурсия графа.
_RECURSION_LIMIT = 80
...
# Поднят под протокол доведения до корня (SYSTEM_PROMPT_RULES п.10): уровневый
# спуск через metric_tree тратит ~1 вызов на уровень дерева, и обход топ-3
# флагов с разложением требует больше вызовов, чем «констатация факта». Широту
# ограничивает топ-3 в промпте, а не этот бюджет.
_TOOL_CALL_BUDGET = 32
