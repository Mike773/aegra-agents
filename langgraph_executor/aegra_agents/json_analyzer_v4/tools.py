...
def build_tools(
    ...
    def peer_context(metric: str, person: str | None = None) -> str:
        ...
        («падает вместе с группой или против неё» — vs_группы), разрыв с
        топ-20% группы, реалистичность плана (hit_rate — какая доля группы
        вообще выполняет план), срезы по уровням иерархии (организация/
        территория/офис — имена как в данных) с локализацией «системное/
        локальное», место и процентиль в рейтинге. Используй для вопросов «как
