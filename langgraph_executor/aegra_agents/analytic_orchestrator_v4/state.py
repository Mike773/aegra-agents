...
Intent = Literal[
    ...
    "analytics",      # конкретный вопрос про метрики (в т.ч. «давай подробнее» о
                      # зоне/достижении, названных в блоке продолжения)
...
class OrchestratorState(OrchestratorOutput, total=False):
    ...
    # Batch-агрегаты peer-групп по уровням (отдельный компонент, те же входные
    # параметры). Опциональны: их сбой не роняет ход — json_analyzer получает их
    # ключом raw_aggregates, нет агрегатов = режим без peer-сравнения. Загрузка
