...
from ..json_analyzer_v4.loader import load_dataset_obj
...
def make_load_data_node():
    async def load_data(state: OrchestratorState, config: RunnableConfig) -> dict:
        ...
        def _fetch() -> tuple[str, dict | None, str | None, Any, str | None]:
            ...
            # Batch-агрегаты peer-групп — отдельный компонент с теми же входными
            # параметрами. Опциональны: их сбой не роняет основную загрузку.
            aggregates: Any = None
            agg_error: str | None = None
            try:
                agg_component = GetBatchAgentAggregateDatasetByFiltersComponent(
                    dataset_name=dataset_name,
                    filters=filters,
                )
                aggregates = agg_component.build_json_output()
            except Exception as exc:  # noqa: BLE001 — внешний клиент, сужать нечем
                agg_error = f"Ошибка при загрузке агрегатов {dataset_name!r}: {exc}"
            return dkey, metrics, error, aggregates, agg_error
...
def make_initial_analysis_node(llm: GigaChat, json_analyzer_graph: Any):
    ...
    факты по метрикам собирает json_analyzer_v4 (без аналитики по бенчмарку,
...
async def _run_analyzer(
    ...
    aggregates опциональны (None безопасен — режим v3 без peer-агрегатов).
    """
    if metrics is None:
