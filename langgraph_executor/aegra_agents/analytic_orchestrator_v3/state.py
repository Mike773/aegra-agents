...
class OrchestratorState(OrchestratorOutput, total=False):
    ...
    # Batch-агрегаты peer-групп по уровням ORG/TERR/OFFICE (отдельный компонент,
    # те же входные параметры). Опциональны: сбой их загрузки не роняет ход —
    # json_analyzer получает их ключом raw_aggregates, нет агрегатов = режим v3.
