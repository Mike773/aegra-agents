...
COLUMN_DOCS: dict[tuple[str, str], str] = {
    ...
    ("v_fact", "element"): (
        "разрез показателя; NULL = строка без разреза (агрегат по всем разрезам). "
        "Смотри агрегат, разрезы — для декомпозиции"
    ),
...
- element IS NULL — строка без разреза; разрезы нужны для декомпозиции."""
