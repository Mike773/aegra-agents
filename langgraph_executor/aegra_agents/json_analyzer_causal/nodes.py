...
def _safe_embed_query(
    ...
    пустой вектор. resolve_entity деградирует до «совпадений нет», но не роняет
    tool-loop — остальные инструменты на SQLite эмбеддингов не требуют."""
...
def make_gather_node(llm: GigaChat):
    ...
    async def gather(state: dict, config: RunnableConfig) -> dict:
        ...
        # Сбой эмбеддингов (rate-limit/сеть) НЕ фатален: они нужны только
        # resolve_entity, а обзор/дерево/флаги работают на SQLite. Деградируем до
        # пустого индекса, чтобы tool-loop отработал, а не падал целиком.
        ...
        # Классификация видов метрик (уровень/вклад/индекс): для знаковых «вкладов»/
        # индексов обнуляем относительные % и пересчитываем вердикт динамики из
        # абсолютного изменения. Сбой изолирован (пусто → прежнее поведение).
