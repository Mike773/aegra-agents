...
def _delta_cell(pct: Any, abs_: Any, unit: str | None) -> str:
    ...
    с единицей (для знаковых/индексных метрик относительный % обнулён)."""
    if pct is not None:
...
def _render_attribution(result: dict[str, Any]) -> str:
    if result.get("error"):
        return _render_error(result)
    method = result.get("method")
    target = result.get("target")
    ttype = result.get("target_type")
    head = [f"Разбор метрики «{target}»" + (f" ({ttype})" if ttype else "") + ":"]
    if method == "anomaly":
        head.append(
            f"Аномальность значения у сотрудника (таб. {result.get('person_tabnum')}) "
            f"на {result.get('date')} относительно коллег "
            f"({result.get('reference_cohort_size')} чел)."
        )
    elif method == "causal":
        head.append(
            f"Период {result.get('date_old')} → {result.get('date_new')}. "
            f"Метод: каузальный (DoWhy, когорта {result.get('cohort_size')} чел)."
        )
    elif method == "algebraic":
        head.append(
            f"Период {result.get('date_old')} → {result.get('date_new')}. "
            "Метод: эвристический (по одному человеку)."
        )
        if result.get("parent_delta") is not None:
            head.append(
                f"Изменение метрики: {_fmt_num(result.get('parent_old'))} → "
                f"{_fmt_num(result.get('parent_new'))} (Δ {_fmt_num(result.get('parent_delta'))})."
            )
    else:
        return _dump(result)

    contribs = result.get("contributions", [])
    body = ["", "Вклад компонентов (по убыванию силы):"]
    for i, c in enumerate(contribs, 1):
        verdict = c.get("verdict") or c.get("child_verdict") or "—"
        body.append(
            f"{i}. {c.get('node')} — доля вклада {_fmt_num(c.get('share_pct'))}%, "
            f"вердикт: {verdict}"
        )
    ...
    if contribs:
        top = contribs[0]
        if method == "anomaly" and top.get("is_self"):
            tail.append(
                "Итог: аномалия НЕ объясняется детьми — это собственный механизм "
                "метрики (причина вне дерева / связь изменилась)."
            )
        else:
            tv = top.get("verdict") or top.get("child_verdict") or "—"
            tail.append(
                f"Главный драйвер: {top.get('node')} "
                f"({_fmt_num(top.get('share_pct'))}%, {tv})."
            )
    if method == "algebraic":
        tail.append(
            "Примечание: эвристика по influent_percent — знак связи ребёнок→родитель "
            "не доказан; для знак-аккуратной атрибуции нужен каузальный режим (когорта)."
        )
    return "\n".join(head + body + tail)
...
def build_tools(
    ...
    def metric_tree(
        ...
        → корнями станут её разрезы (пометка 'разрезы_вместо_агрегата'). Для
        вопроса «почему ИЗМЕНИЛАСЬ» предпочитай attribute_change."""
    ...
    def attribute_change(
        ...
        """ПОЧЕМУ метрика изменилась между периодами: раскладывает изменение
        родительской metric на ВКЛАД дочерних узлов. Вызывай вместо ручного
        сравнения детей при вопросах «почему выросло/просело», «что потянуло».
        Ответ: method ('causal' — DoWhy по когорте; 'algebraic' — эвристика для
        одного человека); contributions отсортированы по силе: node, share_pct,
        verdict ('улучшение'/'ухудшение' с учётом направления — читай его, а не
        знак). person не указан = когорта; person задан = один человек.
        date_old/date_new опц. (по умолчанию два последних периода).
        САМОДОСТАТОЧЕН — не дублируй get_metric/metric_tree по всем людям."""
        metric = _blank_to_none(metric)
        person = _blank_to_none(person)
        date_old = _blank_to_none(date_old)
        date_new = _blank_to_none(date_new)
        if metric is None:
            return _dump({"error": "attribute_change требует точное название метрики"})
        unknown = _unknown_metric(metric) or _unknown_person(person)
        if unknown:
            return unknown
        return _safe(
            _render_attribution,
            causal.attribute_change(
                store, metric, date_old=date_old, date_new=date_new, person=person
            ),
        )
    ...
    def attribute_anomaly(
        ...
        """ПОЧЕМУ значение метрики у сотрудника аномально относительно коллег:
        раскладывает аномальность metric у человека на вклад дочерних узлов (DoWhy
        attribute_anomalies, коллеги — референс). Используй после
        find_flags(kind='anomaly')/is_anomaly=1. person ОБЯЗАТЕЛЕН; date опц.
        contributions: node, share_pct, verdict."""
        metric = _blank_to_none(metric)
        person = _blank_to_none(person)
        date = _blank_to_none(date)
        if metric is None or person is None:
            return _dump(
                {"error": "attribute_anomaly требует точные metric и person"}
            )
        unknown = _unknown_metric(metric) or _unknown_person(person)
        if unknown:
            return unknown
        tab = causal._resolve_person_tabnum(store, person)
        if tab is None:
            return _render_error({"error": f"Человек '{person}' не найден"})
        return _safe(_render_attribution, causal.attribute_anomaly(store, metric, tab, date=date))
