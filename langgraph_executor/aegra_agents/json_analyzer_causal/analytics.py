...
def apply_metric_kinds(store: SqliteStore, kinds: dict[str, str]) -> int:
    ...
    метрики max|fact|). plan_status/benchmark_status уже основаны на знаке абс.
    отклонения и остаются как есть. Виды кладёт в store.metric_kinds. Пустой ввод —
    no-op. Возвращает число затронутых строк."""
