...
подсказкой «не повторяй» — это защита от латентности и зацикливания, а НЕ обход лимита
API: у GigaChat контекст 261120 токенов и отдельного ограничения на запрос с функциями
нет (замер 2026-08-04). Стадия 2 (синтез) — в nodes.make_synthesize_node.
...
# поэтому контекст растёт как сумма всех выдач. Лимиты подобраны так, чтобы пик
# входа GigaChat оставался в рабочем окне: worst-case транскрипт ≈
# _TOOL_CALL_BUDGET × _TOOL_OUTPUT_CAP символов. Для каузального графа сбор обычно
# короткий (attribute_change самодостаточен), поэтому лимиты ужаты сильнее, чем в
# базовом json_analyzer (там 6000/18) — это срезало раздувание контекста с
# десятков тысяч токенов до рабочих значений.
_TOOL_OUTPUT_CAP = 2500
_RECURSION_LIMIT = 50
