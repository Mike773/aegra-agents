...
| `wow_change_abs/pct` | изменение неделя-к-неделе по сериям `(person, metric, element)` |
| `trend` | `рост` / `падение` / `стабильно` (порог ±5 %, первая vs последняя неделя) |
...
`build_summary` — стартовая сводка (охват, средние по метрикам последней недели,
...
| 5 | `compare` | динамика по неделям (требует `person`) |
...
| 10 | `find_flags(kind, ...)` | предрассчитанные строки по силе: `anomaly` / `below_plan` / `above_plan` / `trend` — главный инструмент для широких вопросов |
| 11 | `analytics_summary` | общая сводка |

Важное различие, заложенное в промптах и инструментах: **`trend`** — динамика во
времени («просел / упал»), **`below_plan`** — отклонение от плана («хуже плана»).
Это разные вопросы.
