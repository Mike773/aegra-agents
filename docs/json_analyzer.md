...
- **Двухстадийность.** Сбор данных (`gather`) и выжимка фактов (`synthesize`)
  разделены. Разделение — не обход лимита API (у GigaChat контекст 261120 токенов,
  отдельного ограничения на запрос с функциями нет — замер 2026-08-04), а разнесение
  ролей: стадия 1 ходит инструментами, стадия 2 без функций сжимает весь транскрипт
  в факты с готовыми вердиктами. Прозу по этим фактам пишет вызывающий
  (`BUSINESS_SYSTEM_PROMPT` оркестратора) — правила формулировок живут в одном месте.
