...
| `wow_status` | **вердикт** изменения нед-к-нед с учётом направления: `улучшение` / `ухудшение` / `стабильно` |
| `trend` | `рост` / `падение` / `стабильно` (порог ±5 %, первая vs последняя неделя) — направление ЗНАЧЕНИЯ |
...
`wow_change` и знак `zscore` — это направление **значения**, а не «хорошо/плохо»:
