"""json_analyzer_v5: инструмент peer_context — единый составной рендер peer-данных
...
def test_registered_and_full_render():
    ...
    assert "хуже группы" in out or "на уровне группы" in out
...
def test_no_history_dynamics_without_rel():
    """Без history секция динамики есть (бенчмарк/жёсткость), «против группы» нет."""
    no_history = _payload(
    ...
    assert "отрыв от топ-20%" in out
    assert "жёсткий план" in out
    # Колонка «против группы» пуста во всех строках — из таблицы скрыта (в шапке
    # секции статичное пояснение остаётся).
    assert "| против группы" not in out
...
def test_flat_columns_show_and_hide_peer_verdicts():
    """get_metric: «против группы»/«жёсткость плана» видны при данных и скрыты без них."""
    with_peer = _store(RANKS_TWO_DATES, FULL_AGG)
    out = _tool(with_peer, "get_metric").invoke({"metric": "Продажи"})
    assert "жёсткость плана" in out
    assert "против группы" in out

    without_peer = _store(RANKS_TWO_DATES)
    out2 = _tool(without_peer, "get_metric").invoke({"metric": "Продажи"})
    assert "жёсткость плана" not in out2
    assert "против группы" not in out2
