...
- **Prompt** (`prompts/`): the business prompt is one variable, `BUSINESS_PROMPT` in `prompts/business.py` (role, principles, style, report structure); `compose_system_prompt` orders business prompt → tools guide → schema doc → data blocks → deviation map → memory/briefing → star wording (only when the dataset has stars) → dashboard task hint. `system_prompt_override` replaces only the business prompt. The catalog block shows levels 1-2 and, for each level-2 metric, counters of deeper layers.
