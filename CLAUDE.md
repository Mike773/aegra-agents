...
- **Prompt** (`prompts/`): the business prompt is one variable, `BUSINESS_PROMPT` in `prompts/business.py` (role, principles, style, report structure); `compose_system_prompt` opens with a per-request `query UUID: <uuid4>` line and then orders business prompt → tools guide (cross-cutting rules only; per-tool hints live in each tool's description) → interactive-suggestions block (only with the flag) → data blocks → deviation map → memory → briefing (only for dashboard tasks and the summary, where the model has no dialog history) → star wording (only when the dataset has stars) → dashboard task hint. The SQL schema doc (`db/schema_doc.py`) is **not** in the system prompt: it is the `query_sql` tool description, and its example queries go to GigaChat as the function's `few_shot_examples`. `system_prompt_override` replaces only the business prompt. Data blocks (`db/enrich.py`) are the dataset profile (one line per person: role, post, level-1 metrics with descendant counts, period dates, that person's peer groups), attention zones, the level-1 scoreboard and data gaps; the metric catalog and the peer block are **not** in the prompt (kept only for `scripts/sql_debug.py --enrichment`).
