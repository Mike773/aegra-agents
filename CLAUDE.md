...
```bash
.venv/bin/python scripts/e2e_analyst.py               # analyst_agent on a samples_v2/*.json file, dataset service mocked (E2E_DASHBOARD=1, E2E_PEER=1, E2E_TURNS=2, E2E_WIKI=1, E2E_SUGGESTIONS=1, E2E_CHART=1)
...
- **One LLM loop per turn.** No router LLM, no separate fact-gathering subgraph, no synthesize stage: the model with the business prompt and tools gathers data and writes the final answer itself. `agent/loop.py` is a plain Python loop over `llm.bind_tools` living **inside one node** (aegra runs graphs with the default recursion limit; `langchain-gigachat` serialises only `tool_calls[0]`, so each tool call is one round trip). The loop runs under one GigaChat session (`agent/session.py`, `X-Session-ID` = `thread_id`) so the repeated context prefix is served from GigaChat's cache; `scripts/e2e_analyst.py` prints the cached share.
- **Seven tools** (`tools/`): `query_sql` (free read-only SELECT), `metric_card` (everything about one metric in one call), `peer_context`, `star_rating` (place among peers per org level per quarter; from the person-level `rating` field, which arrives only with stars — level names come from the aggregates), `search_wiki`, `list_deviations`, `note_deviation`. Bound per flags/data. An eighth, `suggest_followups`, exists only with `configurable.interactive_suggestions=true`: the model passes up to 3 `{label, content}` next-question options, and the final message carries them as `additional_kwargs.orchestrator_suggestions` (`[{"type": "message", "label", "content"}]`); texts for that mode live in `prompts/suggestions.py`, and with the flag off nothing changes (no tool, no prompt block, no key). Likewise `build_chart` exists only with `configurable.interactive_chart=true`: the model fills a typed plotly subset (`chart_type`, titles, `traces` of `{name, x, y, type?}`), `tools/chart.py:build_figure` turns it into a plotly figure `{data, layout}`, and the final message carries it as `additional_kwargs.orchestrator_chart = [{"type": "plotly", "data": figure, "tool_call_id": "interactive_chart", "title": <model's title>}]` (always one element, `contract/messages.py:chart_payload`); no call → no chart (code never builds a default one); texts in `prompts/chart.py`.
- **Normalized SQLite** (`db/`): `person`, `metric`, `metric_edge`, `period`, `fact`, `fact_analytics`, `ranking`, `peer_aggregate`, `person_peer`, `rating`, `deviation` + views the agent queries (`v_fact`, `v_fact_latest`, `v_tree`, `v_peer_latest`, `v_star`, `v_rating`, …). `build_run_db()` returns a **ready** DB (data + derived fields + kind-based percent suppression) — callers must not call `compute_analytics` separately.
...
- **Prompt** (`prompts/`): the business prompt is one variable, `BUSINESS_PROMPT` in `prompts/business.py` (role, principles, style, report structure); `compose_system_prompt` opens with a per-request `query UUID: <uuid4>` line and then orders business prompt → tools guide (cross-cutting rules only; per-tool hints live in each tool's description) → interactive-suggestions block, then chart block (each only with its flag) → data blocks → deviation map → memory → briefing (only for dashboard tasks and the summary, where the model has no dialog history) → star wording (only when the dataset has stars) → dashboard task hint. The SQL schema doc (`db/schema_doc.py`) is **not** in the system prompt: it is the `query_sql` tool description, and its example queries go to GigaChat as the function's `few_shot_examples`. `system_prompt_override` replaces only the business prompt. Data blocks (`db/enrich.py`) are the dataset profile (one line per person: role, post, level-1 metrics with descendant counts, period dates, that person's peer groups), attention zones, the level-1 scoreboard and data gaps; the metric catalog and the peer block are **not** in the prompt (kept only for `scripts/sql_debug.py --enrichment`).
