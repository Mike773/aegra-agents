...
- **Prompt** (`prompts/`): v4's `BUSINESS_SYSTEM_PROMPT` split 1:1 into blocks; `compose_system_prompt` orders role → methodology → tools guide → schema doc → data blocks → deviation map → answer rules. `system_prompt_override` replaces only the business part. The catalog block shows levels 1-2 and, for each level-2 metric, counters of deeper layers.
